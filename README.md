...
You need Blender 4.3 or newer (tested with 5.2), a built TiXL Editor with its debug bridge, a TiXL C# operator project, and the .NET SDK. The operator project must be created by TiXL so it has valid release metadata. Start TiXL with `--debug-server 9042` and leave it running.
...
The first save can take a while. Watch `<blend folder>/.tixl_cache/<blend name>/sync_logs/latest.log`. The bridge installs the operators, creates a TiXL project for this `.blend`, builds it, and opens it. TiXL may restart when the operator package or graph changes, so save any open TiXL work first. Later Blender saves rebuild changed content automatically. The **Sync saved .blend to TiXL** button runs the same process on demand.
...
`status --blend ...` checks whether the cache matches the saved file. `--force` rebuilds even when the source hash matches. A full command-line TiXL installation also needs `TIXL_BRIDGE_OPERATOR_PROJECT` and `TIXL_BRIDGE_EDITOR` set to the same folders used in the add-on preferences. The debug bridge uses local port 9042 by default; `TIXL_BRIDGE_PORT` can change the client port.

If a save does not appear in TiXL, check `sync_logs/latest.log` first. Missing camera, missing collections, TiXL not running with its debug server, and a project without release metadata are reported there.
